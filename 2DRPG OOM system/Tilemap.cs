...
using System.Threading.Tasks;
  ...
  public class Tilemap
  {
   ...
   static public int mapSizeX 
    ...
    public string GenerateATestMap(int width, int height) 
    {
        // This is where the char are generate to create the map
        char[,] mapMatrix = new char[width, height];
        ...
        for (int j = 0; j < height; j++)
        {
            for (int i = 0; i < width; i++)
            {
                if (i == 0 || j == 0 || i == width - 1 || j == height - 1)
                    ...
                    mapMatrix[i, j] = '$';  //2nd rule: There will be a Block in these two position (5,4) and (5,5)                
                else
                    mapMatrix[i, j] = '*';  //The rest of the map will be normal map where the player would be able to walk
            }
        }
